...
/* ============================================================
   ...
   Ported 1:1 from design-reference/components/ui.jsx
   ============================================================ */

import { useEffect, useState } from "react";
import { REC } from "@/lib/rubrics";
...
/* ---------------- score helpers ---------------- */
export function scoreTone(v: number): "pos" | "warn" | "neg" {
  return v >= 85 ? "pos" : v >= 70 ? "warn" : "neg";
}
export function toneColor(t: string): string {
  return (
    { pos: "var(--pos)", warn: "var(--warn)", neg: "var(--neg)", info: "var(--info)" }[t] ||
    "var(--ac)"
  );
}
export function scoreColor(v: number): string {
  return toneColor(scoreTone(v));
}

/* ---------------- Avatar ---------------- */
const AV_HUES: Record<string, number> = {
  AR: 245, SN: 290, BS: 200, DL: 160, RP: 20, MA: 320, HW: 55, PA: 130, FN: 0,
};
function hueFromInitials(initials: string): number {
  if (AV_HUES[initials] != null) return AV_HUES[initials];
  let h = 0;
...
export function Avatar({
  ...
  const hue = hueFromInitials(c.initials);
  return (
    <div
      style={{
        width: size,
...
/* ---------------- Status pill ---------------- */
export const STATUS_META: Record<CandidateStatus, { label: string; tone: string }> = {
  ...
  return (
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", textAlign: "center" }}>
        <div>
          <div className="mono" style={{ fontSize: size * 0.3, fontWeight: 600, lineHeight: 1, color: col }}>
            {Math.round(v)}
            {showSuffix && <span style={{ fontSize: size * 0.15, opacity: 0.7 }}>%</span>}
          </div>
          {label && (
            <div style={{ fontSize: size * 0.1, fontWeight: 700, letterSpacing: "0.14em", color: "var(--tx-2)", marginTop: 3 }}>
              {label}
            </div>
          )}
        </div>
