...
import type { Candidate, CandidateStatus, ImprovementSuggestion } from "@/lib/types";
import { NDS } from "@/lib/nds-data";
import { DEFAULT_ROLE_BENCHMARK } from "@/lib/rubrics";
import { Icon, Avatar, RecBadge, StatusPill, ScoreRing } from "./primitives";
import { RadarChart, ScoreBar } from "./charts";

const SCORE_DIMS = [
  { key: "technical" as const, label: "Technical Skills", weight: "30%", icon: "bolt" },
  { key: "certification" as const, label: "Certifications", weight: "20%", icon: "award" },
  { key: "experience" as const, label: "Experience", weight: "25%", icon: "briefcase" },
  { key: "communication" as const, label: "Communication & Leadership", weight: "15%", icon: "user" },
  { key: "quality" as const, label: "Résumé Quality", weight: "10%", icon: "doc" },
];
...
function Stat({ label, value, icon, tone }: { label: string; value: string; icon: string; tone?: string }) {
  return (
    <div style={{ flex: 1, minWidth: 120 }}>
      ...
      <div className="mono" style={{ fontSize: 16, fontWeight: 600, color: tone || "var(--tx-0)" }}>
        {value}
      </div>
    </div>
  );
...
function ReportHeader({
  ...
  return (
    <div className="fade-up" style={{ display: "flex", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
      <button className="btn btn-ghost btn-sm" onClick={onBack} style={{ width: 36, padding: 0 }}>
      ...
      <div style={{ display: "flex", gap: 9, flexWrap: "wrap" }}>
        ...
        <button className="btn btn-primary" onClick={() => onStatus(c.id, "shortlisted")}>
          <Icon name="star" size={16} /> Shortlist
        </button>
      </div>
    </div>
  );
        ...
        {icon && (
          <div
            style={{
              ...
              display: "grid",
              placeItems: "center",
              background: `color-mix(in srgb, ${iconTone || "var(--ac)"} 14%, transparent)`,
              color: iconTone || "var(--ac-hi)",
            }}
          >
...
const sugMeta: Record<string, { icon: string; tone: string; label: string }> = {
  add: { icon: "plus", tone: "var(--ac)", label: "Project" },
  cert: { icon: "award", tone: "var(--warn)", label: "Certification" },
  kw: { icon: "search", tone: "var(--info)", label: "Keywords" },
  word: { icon: "doc", tone: "var(--pos)", label: "Wording" },
};
  ...
  const suggestions: ImprovementSuggestion[] =
    c.suggestions && c.suggestions.length > 0
      ...
      : [
          {
            type: "word",
            text: "Quantify more outcomes with metrics to strengthen the profile.",
          },
        ];

  const forwardLabel =
    c.recommendation === "STRONG" || c.recommendation === "HIRE"
      ? "Yes — forward to hiring manager"
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <ReportHeader
        ...
        onStatus={onStatus}
        onCompare={onCompare}
        inCompare={compareSet.includes(c.id)}
      />
      ...
      <div
        ...
        style={{
          ...
          background: "linear-gradient(120deg, var(--bg-2), color-mix(in srgb, var(--ac) 8%, var(--bg-2)))",
          animationDelay: "30ms",
        }}
      >
        ...
        <div style={{ fontSize: 16, fontWeight: 700, color: "var(--tx-0)" }}>{forwardLabel}</div>
        <div style={{ fontSize: 13, color: "var(--tx-2)", marginTop: 4 }}>
          Should HR forward this candidate to the hiring manager?
        </div>
      </div>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 260 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10, flexWrap: "wrap" }}>
            ...
            <span className="chip">
              <Icon name="sparkle" size={13} style={{ color: "var(--ac-hi)" }} /> {c.confidence}% confidence
            </span>
        ...
        >
          ...
          <Stat
            label="Notice"
            value={`${c.noticeDays} days`}
            icon="clock"
            tone={c.noticeDays > 45 ? "var(--warn)" : "var(--tx-0)"}
          />
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 18 }} className="report-grid">
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          <CardBox
            ...
            sub="Weighted across the SI scoring model"
            icon="bolt"
          ...
          <CardBox
            title="AI Improvement Assistant"
            sub="Targeted recommendations to raise this candidate's fit"
            icon="sparkle"
            iconTone="var(--ac)"
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <RevealList
                items={suggestions}
                render={(s) => {
                  const m = sugMeta[s.type] || sugMeta.word;
                  return (
                    <div
                      style={{
                        display: "flex",
            ...
            <CardBox title="Strengths" icon="check" iconTone="var(--pos)">
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {c.highlights.map((h, i) => (
                  ...
                  >
                    <Icon name="check" size={15} stroke={2.4} style={{ color: "var(--pos)", flexShrink: 0, marginTop: 1 }} />
                    {h}
                  </div>
                ))}
              </div>
            ...
            <CardBox title="Gaps & Risks" icon="flag" iconTone="var(--warn)">
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {c.gaps.map((g, i) => (
                  ...
                  >
                    <Icon name="flag" size={14} style={{ color: "var(--warn)", flexShrink: 0, marginTop: 1 }} />
                    {g}
                  </div>
                ))}
              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
          <CardBox title="Capability Radar" sub="Profile vs. role benchmark" icon="analytics">
            <div style={{ display: "grid", placeItems: "center", padding: "4px 0 10px" }}>
              <RadarChart
                ...
                axes={NDS.RADAR_AXES}
                size={272}
                compare={DEFAULT_ROLE_BENCHMARK}
              />
            </div>
            <div
              style={{
            ...
            >
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                <span style={{ width: 12, height: 3, borderRadius: 2, background: "var(--ac)" }} />
                Candidate
              </span>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                <span style={{ width: 12, height: 0, borderTop: "2px dashed var(--info)" }} />
                Role benchmark
              </span>
            </div>
          </CardBox>
          ...
          <CardBox
            ...
            sub={`${c.certs.filter((x) => x.active).length} active · ${c.certs.filter((x) => !x.active).length} expired`}
            icon="award"
            iconTone="var(--warn)"
          >
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      padding: "2px 8px",
                      borderRadius: 5,
                      color: ct.active ? "var(--pos)" : "var(--neg)",
                      background: ct.active ? "var(--pos-soft)" : "var(--neg-soft)",
                    }}
                  >
          ...
          <CardBox title="Interview Recommendation" icon="user" iconTone="var(--ac)">
            <p style={{ fontSize: 12.5, color: "var(--tx-1)", lineHeight: 1.55 }}>
              Recommend a <b style={{ color: "var(--tx-0)" }}>technical panel + presales scenario</b>. Probe cloud
